...
[p]Dobra, do rzeczy. Wyobraźmy sobie, że mamy sobie [url=https://tutorials.comandeer.pl/res/html5-blog/start.html]przykładowy blog[/url]:[/p]
...
[p]Nic nadzwyczajnego – blog jakich dużo w Internecie. Przyznam się bez bicia, że ten przykładzik to połączenie blogu [url=http://lea.verou.me]Lea Verou[/url] i naszego kochanego [url=http://web.archive.org/web/20150901161552/http://roberto.ovh.org:80/blog]Roberto[/url]. Kodu nie będę wyjaśniał, bo mam wrażenie, że powinien być jasny. Ot, zwyczajny wpis na zwyczajnym blogu, poniżej komentarze, formularz do komentowania i jakieś tam dodatkowe info w bocznym panelu. Specjalnie nie dorzucam CSS, bo jestem z tych purystów, co to twierdzą, że strona to treść i kod, a wygląd jest jedynie dodatkiem.[/p]
...
[p]W przypadku skryptów sprawa jest nieco bardziej skomplikowana. Jeśli mówimy o "normalnym" skrypcie (np. dołączenie jQuery, slider), wówczas [tt][type][/tt] jest całkowicie zbędne – i tak jest to zawsze [tt][type=text/javascript][/tt]. W specyfikacji są podane jednak także inne sposoby wykorzystania [tt]script[/tt], m.in. jako [url=https://stackoverflow.com/questions/4912586/explanation-of-script-type-text-template-script]systemu szablonów[/url]. Ostatnio także [url=https://html.spec.whatwg.org/multipage/webappapis.html#module-script]dodano do specyfikacji [tt][type=module][/tt][/url], które pozwala [url=http://exploringjs.com/es6/ch_modules.html]wczytywać moduły ES[/url].[/p]


[h2="obsluga-starszych-przegladarek"]Obsługa starszych przeglądarek[/h2]
[p]Niestety, z obsługą reszty znaczników nie jest już tak kolorowo i stare IE wymagają odpowiedniego skryptu, który trza dodać w [tt]head[/tt] (IE 9 na szczęście jest już normalny):[/p]
[code=markup]<!--[if lt IE 9]>
	<script src="https://cdn.jsdelivr.net/npm/html5shiv@3.7.3/dist/html5shiv-printshiv.min.js"></script>
<![endif]--> [/code]
[p]Po więcej informacji o tym skrypciku zapraszam do [url=https://github.com/aFarkas/html5shiv]oficjalnego repozytorium na GitHubie[/url], na [url=https://remysharp.com/2009/01/07/html5-enabling-script/]blog Remy'iego Sharpa[/url] i [url=https://www.paulirish.com/2011/the-history-of-the-html5-shiv/]blog Paula Irisha[/url]. Jeśli natomiast w przyszłości wzbogacisz swój blog o jakieś nowinki z HTML 5 APIs (np. nagrywanie video komentarzy), to warto zapoznać się też z pojęciem feature detection (ang. wykrywanie funkcjonalności) i bibliotekami [url=https://modernizr.com/]Modernizr[/url] czy [url=https://github.com/phiggins42/has.js/]has.js[/url]. Dodatkowo można też wykorzystać [url=https://polyfill.io/v2/docs/]usługę Polyfill.io[/url], która sama decyduje, jakie polyfille są potrzebne i je dołącza. Warto także poczytać o [url=http://responsivenews.co.uk/post/18948466399/cutting-the-mustard]technice [q]cut the mustard[/q][/url].[/p]
[p]Tutaj warto też od razu zwrócić uwagę na fakt, że nieznane przeglądarce elementy są nieostylowane w żaden sensowny sposób i przez to możemy dostać np. liniowe [tt]section[/tt], co raczej jest niezbyt pożądane. Na szczęście problem ten rozwiązuje normalizacja stylów (nawet jeśli nie chcesz używać nowych znaczników, to i tak powinieneś rozważyć [url=http://nicolasgallagher.com/about-normalize-css/] normalizację[/url] lub [url=http://html5doctor.com/html-5-reset-stylesheet/]resetowanie[/url]), przypisując najbardziej podstawowe style odpowiednim elementom.[/p]
[p=info]Jeśli się zastanawiasz [q]a co z użytkownikami IE 9-, którym nie działa JS?[/q], odpowiem tak: jest ich na tyle mało, że nie sądzę, byś kiedykolwiek na nich trafił. Niemniej istnieją dwie alternatywy dla znaczników HTML5 (na chwilę obecną już bym ich nie polecał): [url=https://tutorials.comandeer.pl/html5-blog.html#aria-role]używanie [tt]div[role][/tt][/url] lub [url=http://2ndidea.com/en/html5-coding-without-html5shiv/](prawie) nieużywanie znaczników HTML5[/url]. Warto się także zastanowić, czy nie porzucić całkowicie HTML5 Shiva, jeśli mamy pewność, że użytkownicy [i]naprawdę[/i] starych IE nam się nie trafią.[/p]
[p=info]Warto także wspomnieć o [url=https://www.quirksmode.org/css/condcom.html]komentarzach warunkowych dla IE[/url]. Niemniej w IE 10 już nie działają, a prawdę mówiąc w dzisiejszym webdevie istnieją o wiele lepsze metody (jak wspomniane wyżej feature detection).[/p]
...
[quote="https://w3c.github.io/html/dom.html#element-attrdef-global-id"]
[p]Identifiers are [b]opaque strings[/b]. Particular meanings should not be derived from the value of the [tt]id[/tt] attribute.[/p]
[p][Identyfikatory są [b]ciągami nieprzejrzystymi[/b]. Konkretne znaczenia nie powinny być wywodzone z wartości atrybutu [tt][id][/tt].][/p]
[/quote]
[p]HTML5 rozwiązuje ten problem, wprowadzając znacznik [tt]header[/tt]. Jak sama nazwa wskazuje, służy on do oznaczania nagłówków i świetnie się nadaje do oznaczenia nagłówka strony. Zatem zamieńmy nasz [tt]div#header[/tt] na [tt]header#header[/tt]. Niby mała zmiana, a semantyka kodu rośnie. Ale, ale – to jeszcze nie wszystko! Spójrzmy na nagłówek: mamy tam [tt]h1[/tt] z tytułem strony i [tt]p[/tt] z mottem. Fajnie by było gdyby motto też było nagłówkiem, ale co z hierarchią nagłówków? Czy przypadkiem się nie zaburzy? Na chwilę obecną musisz wiedzieć, że podtytuły, zgodnie z tym, co [url=https://w3c.github.io/html/common-idioms-without-dedicated-elements.html#subheadings-subtitles-alternative-titles-and-taglines]czytamy w specyfikacji[/url], winny być oznaczane przy pomocy [tt]p[/tt], tudzież [tt]h* > span[/tt]. Do samej hierarchii nagłówków [url=https://tutorials.comandeer.pl/html5-blog.html#artykul-outline]jeszcze wrócimy[/url].[/p]
...
[p=warning]Co prawda w HTML5 [i]istniał[/i] także element [tt]menu[/tt], ale służył on do tworzenia menu dla aplikacji (menu kontekstowe czy toolbary) i nie należy go mylić z [tt]nav[/tt].[/p]
...
[quote]
[p]But what would be the purpose of marking it [main content] up specifically, anyway? If you need to style it, use a div. An assistive technology like a screenreader can find the main content because it is the first thing inside a page that isn’t a header, nav or footer.[/p]
[p][Ale jaki mielibyśmy cel w dodatkowym oznaczaniu głównej treści? Jeśli potrzebujesz ją ostylować, użyj diva. Technologia asystująca, taka jak czytnik ekranowy, będzie w stanie znaleźć główną treść z tego prostego względu, że będzie pierwszą rzeczą wewnątrz strony, która nie jest ani nagłówkiem, ani stopką.][/p]
[/quote]
[p]Niemniej potrzeba oznaczenia głównej treści strony stała się na tyle duża, że [url=https://w3c.github.io/html/grouping-content.html#the-main-element]dodano do HTML5 nowy znacznik, służący wyłącznie temu – [tt]main[/tt][/url]. Warto dodać, że ten znacznik – ze względów dostępności – powinien znaleźć się na stronie [b]wyłącznie raz[/b], co powoduje [url=https://github.com/whatwg/html/issues/100]niekończący się spór między WHATWG i W3C[/url] (tutaj warto zwrócić uwagę na to, że [url=https://medium.com/content-uneditable/the-great-world-of-open-web-standards-64c1fe53063]istnieją dwa standardy HTML[/url] – w tym tutorialu będziemy się opierać głównie na wersji W3C jako tej, która w dogłębniejszy sposób zajmuje się problemami semantyki i dostępności).[/p]
...
[p=info]Mała dygresja: [tt][alt][/tt] jest potrzebny dla obrazka z powodów dostępności. Jednakże czasami… warto pozostawić go pustym, z tych samych powodów. O co chodzi? Otóż nie ma sensu powielać opisu obrazka, jeśli jest on zawarty w otaczającej go treści (np. jeśli w [tt]figcaption[/tt] jest dokładny opis tego, co na obrazku, należy zostawić [tt][alt][/tt] pusty; często zdarza się jednak, że w takim wypadku [tt][alt][/tt] opisuje co jest na obrazku, a [tt]figcaption[/tt] dodaje do tego interpretację/wyjaśnienie; np [tt][alt][/tt] – [q]czerwony ptak na gałęzi[/q], [tt]figcaption[/tt] – [q]Karłowata odmiana papugi[/q]). W takich wypadkach można użyć [tt][aria-labelledby][/tt]. W3C przygotowało [url=https://w3c.github.io/html/semantics-embedded-content.html#alt-text]kilka wskazówek[/url] (zresztą [url=https://webaim.org/techniques/alttext/]nie tylko ono[/url]). Ciekawy jest także fakt, że [url=https://developer.paciellogroup.com/blog/2014/04/short-note-alt/][tt][alt][/tt] to nie jedyny sposób na zapewnienie dostępności obrazków[/url].[/p]
...
[p]Warto zauważyć, że rozwojem technik związanych z responsywnością zajmuje się powołana [url=https://www.w3.org/community/respimg/]community group [grupa społeczna][/url] (która jest teraz częścią [url=https://www.w3.org/community/wicg/]większej inicjatywy[/url]). To właśnie ona stoi za powstaniem tagu [tt]picture[/tt] oraz jego mniej inwazyjnych odpowiedników: [url=https://html.spec.whatwg.org/multipage/embedded-content.html#srcset-attributes]atrybutów [tt][srcset][/tt] i [tt][sizes][/tt] dla tagu [tt]img[/tt][/url]. Warto też nadmienić, że WHATWG odrzuciło początkowo propozycję znacznika [tt]picture[/tt], promując [tt][srcset][/tt]. Najlepiej wyjaśnia to Ian Hickson - [s]najbardziej wpływowy człowiek w Sieci[/s] (współ)redaktor żyjącego standardu:[/p]
[quote="http://html5doctor.com/interview-with-ian-hickson-html-editor/"]
[p]The way we usually approach ideas is that we describe the problem, then we come up with solutions to the problem and compare them, by evaluating them against the problem. This is where most concrete proposals run into a “no”; something else got the “yes” instead. [tt]<picture>[/tt] is an example of this: we described the problem (that took a few months of back and forth), and then once we had a problem description, I looked at the various proposals, and synthesised a solution based on those that addressed the problems adequately while trying to avoid some common design pitfalls based on lessons we’d learnt from previous ideas. So for example with [tt]<picture>[/tt], we learnt with [tt]<video>[/tt] and [tt]<source>[/tt] that having multiple elements for selecting a resource is a huge design pitfall, so when designing a solution to the problem here, I avoided that (hence the [tt]srcset=""[/tt] design based on CSS’s [tt]image-set()[/tt] proposal).[/p]
[p][Najczęściej podchodzimy do pomysłów poprzez uprzednie opisaniu problemu, a następnie znalezienie jego rozwiązań i ich porównanie ze sobą w zakresie tego, jak są w stanie rozwiązać problem. To właśnie moment, w którym najwięcej konkretnych propozycji natrafia na "nie"; niektóre jednak natrafiają na "tak". [tt]<picture>[/tt] jest przykładem tego: opisaliśmy problem (to zajęło nam kilka miesięcy), a gdy już mieliśmy jego dokładny opis, przeglądnąłem różne propozycje i stworzyłem rozwiązanie mające omijać znane błędy projektowe z wcześniejszych rozwiązań. W przypadku [tt]<picture>[/tt] były to doświadczenia z [tt]<video>[/tt] i [tt]<source>[/tt], wskazujące na to, że posiadanie wielu elementów służących do wyboru konkretnego zasobu jest wielkim błędem projektowym, toteż, tworząc nowe rozwiązanie, unikałem tego (stąd [tt]srcset=""[/tt]), oparte na CSS-owej propozycji [tt]image-set()[/tt]).][/p]
[/quote]
...
[quote="http://html5doctor.com/interview-with-ian-hickson-html-editor/"]
[p]In reality, I can’t really say either “yes” or “no”. What I say doesn’t mean anything: I don’t write any browsers. The browser vendors say “no” to me all the time, by just not implementing what I’ve specced.[/p]
[p][W rzeczywistości nie mogę tak naprawdę powiedzieć ani "tak", ani "nie". To, co mówię, nic nie nzaczy: nie tworzę przeglądarek. Twórcy przeglądarek cały czas mówią mi "nie", po prostu nie implementując tego, co ja umieszczam w specyfikacji.][/p]
[/quote]
[p]Rzeczywistość szybko zweryfikowała poglądy Hixiego i obecnie [url=https://html.spec.whatwg.org/multipage/embedded-content.html#the-picture-element][tt]picture[/tt] już jest w HTML Living Standard[/url]. Tym samym w cień odeszło [url=https://tabatkins.github.io/specs/respimg]nowe rozwiązanie – [tt][src-n][/tt][/url], które łączy w sobie niejako [tt]picture[/tt] i [tt][srcset][/tt] w prostsze, bardziej zrozumiałe rozwiązanie. Coraz częściej próbuje się także zrzucić odpowiedzialność za rozwiązanie problemu [url=https://www.igvita.com/2013/08/29/automating-dpr-switching-with-client-hints/]na serwer[/url] (co IMO jest bardzo rozsądnym rozwiązaniem). Jednak najlepszym (ale też najbardziej długotrwałym) rozwiązaniem wydaje się po prostu [url=https://www.smashingmagazine.com/2013/09/responsive-image-container/]stworzenie nowego formatu obrazków[/url]. Zresztą nieważne tak naprawdę, jak problem responsywnych obrazków zostanie rozwiązany – najważniejsze, że(by?) został rozwiązany.[/p]

[h3="artykul-outline"]Outline[/h3]
[p]Dobra, akapitów się (jeszcze) zmienić nie da, więc treść mamy z głowy. Przyjrzyjmy się zatem hierarchii nagłówków w HTML5. Jest to o tyle ważny temat, że hierarchia nagłówków odzwierciedla całą strukturę treści na stronie (stanowi jej spis treści) i dzięki temu [url=http://internet-bez-barier.com/elementy-naglowkowe-a-dostepnosc/#naglowki-w-nawigacji]ułatwia nawigację dla osób korzystających z czytników ekranowych[/url].[/p]
[p]W HTML5 istniał ambitny plan odejścia od tradycyjnych znaczników [tt]h1[/tt]–[tt]h6[/tt] i przejścia na [url=https://w3c.github.io/html/sections.html#creating-an-outline]hierarchię treści opartą na nowych znacznikach sekcjonujących[/url] ([tt]section, article[/tt] itd.). Niemniej [url=https://www.paciellogroup.com/blog/2013/10/html5-document-outline/]nikt tego nie zaimplementował[/url] i ostatecznie [url=https://lists.w3.org/Archives/Public/public-html/2016Apr/0032.html]postanowiono pozbyć się tej fikcji[/url]. Stąd jedynym poprawnym sposobem tworzenia hierarchii treści na stronie HTML5 jest połączenie obydwu metod (tagi sekcjonujące + różnicowanie ważności nagłówków), gdyż nowy algorytm zostanie usunięty ze specyfikacji i zastosowanie go spowoduje, że [url=http://en.wikipedia.org/wiki/Assistive_Technology]AT[/url] dostaną spłaszczoną hierarchię nagłówków (co raczej jest średnim pomysłem).[/p]
[p]Jeśli chcesz przetestować swój outline, wystarczy zaznaczyć odpowiednią opcję ([q]Show outline[/q]) w [url=https://validator.w3.org/nu]walidatorze[/url].[/p]
[p=info]Jako ciekawostkę można tutaj powiedzieć, że umiejętnie korzystając z atrybutu [tt][role=heading][/tt] i [tt][aria-level][/tt] można tworzyć hierarchię na samych [tt]h1[/tt], tak, jak zostało to opisane w specyfikacji. Metodę tę stosuje [url=https://github.com/ThePacielloGroup/html5-h]niestandardowy znacznik [tt]html5-h[/tt][/url] (przy pomocy [url=https://webroad.pl/javascript/3505-web-components]standardu Web Components[/url] i [url=https://www.polymer-project.org/1.0/]frameworka Polymer[/url], z którym [url=https://tutorials.comandeer.pl/polymer.html]miałem już do czynienia[/url]). Niemniej warto zauważyć, że to bardziej hack niźli eleganckie rozwiązanie problemu. Jednakowoż [tt][aria-level][/tt] można użyć w przypadku, gdy potrzebujemy więcej niż 6 poziomów zagłębień nagłówków (do "pogłębienia" [tt]h6[/tt] – bardzo edge'owy przypadek, ale warto o tym wiedzieć).[/p]
[p]Jak zatem stworzyć sensowną hierarchię nagłówków? Wystarczy trzymać się kilku prostych zasad:[/p]
[list]
	[*] Nagłówki powinny być po kolei, zatem najpierw [tt]h1[/tt], potem [tt]h2[/tt], [tt]h3[/tt] itd. Nie powinna wystąpić sytuacja, gdy hierarchia zaczyna się od np. [tt]h4[/tt] albo gdy po [tt]h2[/tt] wystąpi od razu [tt]h5[/tt].
	[*] Nagłówek [tt]h1[/tt] powinien wystąpić na stronie tylko raz i powinien stanowić [url=https://blog.comandeer.pl/html-css/a11y/2017/07/04/o-naglowkach-slow-kilka.html#g%C5%82%C3%B3wny-nag%C5%82%C3%B3wek]główny nagłówek strony[/url].
	[*] Każda sekcja lub artykuł powinny mieć swój nagłówek.
[/list]
[p]Przyjrzyjmy się nieco bliżej kwestii głównego nagłówka. W przypadku bloga tego typu nagłówek będzie się bowiem różnił pomiędzy stroną główną a stronami poszczególnych wpisów. Na stronie głównej bowiem głównym nagłówkiem będzie nazwa strony, zatem kod będzie wyglądał mniej więcej tak:[/p]
[code=markup]<header id="header">
	<h1><a href="http://example.net">Example.net - fajowy blog, na którym bloguję</a></h1>
	<p>Motto</p>
	[…]
</header>[/code]
[p]W przypadku jednak strony konkretnego wpisu głównym nagłówkiem powinien być tytuł wpisu, bo to on informuje, o czym jest dana strona. Dlatego też nagłówek w [tt]#header[/tt] nie pełni już swojej funkcji, stając się wyłącznie linkiem nawigacyjnym:[/p]
[code=markup]<header id="header">
	<a href="http://example.net">Example.net - fajowy blog, na którym bloguję</a>
	<p>Motto</p>
	[…]
</header>[/code]
[p]Natomiast [tt]h1[/tt] znajdzie się wówczas w [tt]main > article[/tt]:[/p]
[code=markup]<main>
	<article>
		<header>
...
[p=info]Co uważniejszy czytelnik zapewne zastanowi się teraz, czy aby na pewno [tt]body > header > h1[/tt] to to samo co [tt]article h1[/tt]. Odpowiedź na to pytanie zależy. Na gruncie semantyki nie, ponieważ pierwszy nagłówek jest nagłówkiem strony, a drugi - nagłówkiem konkretnego artykułu. Niemniej na gruncie obecnej implementacji dostępności (czyli braku implementacji algorytmu outline'u) ranga nagłówków [b]nie zależy[/b] od semantyki. Tym samym [tt]h1[/tt] [b]zawsze[/b] jest najważniejszym nagłówkiem na stronie, bez względu na miejsce wystąpienia. I dopóki algorytm outline'u nie zostanie zaimplementowany, tego typu rozwiązanie ([tt]article > h1[/tt] jako główny nagłówek strony) wydaje się najlepsze – pomimo swojej niedoskonałej semantyki.[/p]
[p]Można pójść o krok dalej. Heydon Pickering w swojej książce [url=https://www.smashingmagazine.com/inclusive-design-patterns/]Inclusive Design Patterns[/url] stwierdza, że w chwili, gdy logo strony nie jest nagłówkiem a jedynie linkiem, staje się integralną częścią nawigacji. Ma to sens, gdyż logo jest odnośnikiem do strony głównej. Tym samym na stronach poszczególnych wpisów można się pokusić o mocniejsze przebudowanie nagłówka:[/p]
[code=markup]<header id="header">
	<nav>
		<ul>
			<li>
...
[p]W tej wersji logo strony staje się pierwszym elementem menu.[/p]
[p=info]Jak w praktyce może wyglądać zamiana nagłówków miejscami na stronie głównej i stronach wpisów, można zobaczyć na [url=http://internet-bez-barier.com/]Internecie bez barier[/url] lub [url=https://www.webkrytyk.pl/]WebKrytyku[/url]. Z kolei wzorzec z [tt]nav[/tt] można zobaczyć na [url=https://www.heydonworks.com/]stronie domowej Heydona[/url].[/p]

[h3="artykul-komentarze"]Komentarze[/h3]
[p]OK, to jeszcze słówko na temat wyświetlania komentarzy: w HTML5 w końcu [url=http://html5doctor.com/cite-and-blockquote-reloaded/][tt]cite[/tt] działa tak, jak nakazuje logika[/url] i nie ogranicza sie już tylko i wyłącznie do oznaczania tytułów dzieł, ale także innego rodzaju źródeł, w tym osób. To sprawia, że komentarze można traktować jako cytaty wypowiedzi konkretnych osób![/p]
[p]Niemniej ważniejszą kwestią jest sposób organizacji komentarzy. [url=https://w3c.github.io/html/sections.html#example-86aedc40]Przykład w specyfikacji[/url] jest bardzo sensownym punktem wyjścia. Pokazuje on, że komentarze znajdują się w podsekcji ([tt]article > section[/tt]), a dodatkowo są umieszczone w liście uporządkowanej ([tt]ol[/tt]):[/p]
[code=markup]<section id="komentarze" class="comments">
	<h2>Komentarze</h2>
	<ol>
...
[p]Specyfikacja HTML 5.0 sugerowała [url=https://www.w3.org/TR/html50/sections.html#article-example]oznaczanie komentarzy przy pomocy [tt]article[/tt][/url]. Dzięki temu powstała swoista hierarchia artykułów (tzn. każdy komentarz był "podartykułem" wpisu). Jest to logiczne, ponieważ każdy komentarz może być wyświetlany "samodzielnie" (wszak opinia o dziele A może istnieć odrębnie od tego dzieła). Przywodzi to na myśl stare, dobre gazety, w których odpowiedzią na konkretny artykuł jest… inny artykuł.[/p]
[p]Jeśli jednak już zgodzimy się na oznaczanie komentarzy przy pomocy artykułu, czekają nas kolejne wyzwania. Otóż regułą kciuka jest, by każdy artykuł miał odpowiedni nagłówek. Lecz jaki nagłówek powinien mieć komentarz? Na ForumWeb.pl wywiązała się [url=http://www.forumweb.pl/komentarze-kurshtml-edu-pl/html-html5/496603#496603]gorąca dyskusja na ten temat[/url], która skłoniła mnie do refleksji na temat kodu komentarzy. Na większości stron w Sieci, a także [url=https://www.w3.org/TR/html50/sections.html#article-example]w specyfikacji[/url], komentarze są traktowane jako wyjątek, który nie posiada nagłówka (bo są to artykuły zagnieżdżone). Niemniej na chwilę obecną uważam, że dodanie jako nagłówka nazwy użytkownika nie zaszkodzi – pozwoli to nam w łatwiejszy sposób śledzić komentarze danego użytkownika (np. naszego ulubionego hejtera!). Pomysł, by w nagłówku czy etykietce [tt][aria-label][/tt] dodawać pierwsze słowa komentarza ostatecznie odrzuciłem – z tego samego powodu, z jakiego niekiedy warto zostawić puste [tt][alt][/tt]: nie ma sensu dublować treści.[/p]
...
[p]Ostatecznie kod komentarzy w oparciu o [tt]article[/tt] wygląda tak:[/p]
...
[quote="http://developers.whatwg.org/sections.html#the-article-element"]
[p]The [tt]<article>[/tt] element represents [b]a complete, or self-contained, composition in a document[/b], page, application, or site. This could be a magazine, newspaper, technical or scholarly article, an essay or report, a blog or other social media post.[/p]
[p][Element [tt]<article>[/tt] reprezentuje [b]kompletną, niezależną i samodzielną kompozycję w dokumencie[/b], stronie, aplikacji lub witrynie. Może to być magazyn, gazeta, techniczny lub szkolny artykuł, esej lub raport, blog lub inny wpis z sieci społecznych.][/p]
[/quote]
[p]Najważniejszy jest pogrubiony fragment. Ba, tylko co on znaczy? Idąc za [url=http://html5doctor.com/the-article-element/]radą doktorów[/url] trza pomyśleć, czy dany fragment wyciągnięty z kontekstu wciąż ma jakiś sens. Notka o autorze pozbawiona sąsiedztwa wpisu raczej nie stanie się bełkotem. Lista archiwalnych wpisów również pozostanie listą, tak jak lista tagów – listą tagów. Dlatego, moim skromnym zdaniem, chociaż na pierwszy rzut oka wydaje się to pewnym nadużyciem, [tt]article[/tt] pasują tutaj idealnie. Rozważania te wspiera wyżej cytowany opis [tt]article[/tt], który pojawił się po raz pierwszy w specyfikacji HTML 5.1, jeszcze bardziej zwracający uwagę na kwestię niezależności danego fragmentu treści.[/p]
[p]Szkielet:[/p]
[code=markup]<aside>
	<article>
		<h3>O autorze</h3>
...
[p=warning][b]Nie powinien być jednak traktowany jako zastępstwo dla [tt]label[/tt][/b]. [url=https://w3c.github.io/html/sec-forms.html#the-placeholder-attribute]Wspomina o tym nawet specyfikacja[/url].[/p]
...
[p][tt]label[/tt] do pola wyszukiwania można wstawić [url=https://adaptivethemes.com/using-css-clip-as-an-accessible-method-of-hiding-content/]techniką ukrywania przed graficznymi przeglądarkami[/url], używaną m.in. w [url=https://html5boilerplate.com/]HTML5 Boilerplate[/url]. Userzy z wizualnymi przeglądarkami nie zobaczą go (a pole wyszukiwania jest na tyle charakterystyczne, że na pewno je rozpoznają), natomiast ci, którzy korzystają z czytników ekranowych, nie poczują się zagubieni, gdy natrafią na pole bez etykiety.[/p]
...
[p]Oznacza to, że możesz być pewny tylko tego, że końcowy user zobaczy treść i HTML (patrz: użytkownicy Lynxa). Stąd ważne jest zrozumienie podstaw tworzenia czystego, semantycznego kodu, który – nawet bez CSS – tworzy sensowną i czytelną całość. Dlatego też osobiście wolę tworzyć najpierw HTML dla strony, a dopiero później dorabiać CSS i JS. HTML to kartka papieru, na którą nakładam kolejne warstwy folii (CSS, JS). Podejście to znane jest jako [url=https://webroad.pl/inne/3722-progressive-enhancement-zapomniany-fundament]Progressive Enhancement[/url].[/p]

[h3="formularze-kompatybilnosc"]Kompatybilność[/h3]
[p]Teraz zapewne się zapytasz: [q]a gdzie nowe typy pól formularzy są obsługiwane?[/q]. Zamiast podawać tak szybko deaktualizujące się informację, [url=http://caniuse.com/#feat=forms]odsyłam do odpowiedniego źródła[/url]. Brak wsparcia nie oznacza jednak, że trza rezygnować z nowoczesnych formularzy. HTML5 jest pisany z myślą o kompatybilności wstecznej, co znaczy, że jeśli dany typ pola nie jest obsługiwany, pojawi się stare, dobre pole tekstowe. Resztę zachowań – włącznie z walidacją wprowadzonych danych – we wszystkich przeglądarkach [url=https://www.thecssninja.com/javascript/h5f]można osiągnąć dzięki JS[/url].[/p]
[p]Przeróbmy zatem formularz dodawania komentarzy:[/p]
...
[p]Tutaj warto wspomnieć o jeszcze jednym nowym atrybucie dla pól formularza: [tt][pattern][/tt], który przyjmuje jako wartość [url=https://www.regular-expressions.info/]wyrażenie regularne[/url], dzięki czemu – jeśli żadne z nowych typów pól formularza nie spełnia naszych wymagań – można wymusić kokretny format danych, np kodu pocztowego:[/p]
[code=markup]<input type="text" name="zipcode" pattern="\d{2}\-\d{3}">[/code]
[p]Naprawdę zachęcam do używania nowych typów formularzy. Dzięki nim można uprościć walidację danych po stronie serwera (a przynajmniej nie martwić się, że w e-mailu dostajemy JS-a).[/p]
[p=warning]Uprościć, [b]nie znaczy[/b] zaniechać. Niewalidowanie danych po stronie serwera jest jak wpuszczanie nieznajomego do domu, dając mu do tego klucz do sejfu w sypialni. Sama specyfikacja wspomina, że [url=https://w3c.github.io/html/sec-forms.html#constraints-security][b]walidacja po stronie klienta ma jedynie podnosić UX[/b][/url].[/p]
...
[*] [tt]archive[/tt]/[tt]archives[/tt] – znany z Wordpressa, prowadzi do archiwum wiadomości. Typ ten niegdyś występował w specyfikacji HTML5, jednak obecnie nie jest nawet typem proponowanym przez Microformats wiki. Niemniej znajduje się w sekcji [url=http://microformats.org/wiki/existing-rel-values#POSH_usage]wykorzystywanych wartości[/url]. Osobiście wydaje mi się, że w końcu zostanie przeforsowany, dlatego zostawiam go w kodzie bloga (pomimo, że walidator się rzuca – ale na szczęście to [url=https://webroad.pl/html5-css3/3925-validate-or-not-to-validate-that-is-the-question]nie nieomylny demiurg[/url])
...
[p=warning]Przy walidacji formularzy również warto użyć ARIA, zwłaszcza dołączając obok atrybutu [tt][required][/tt] atrybut [tt][aria-required][/tt] oraz oznaczając niepoprawnie wypełnione pola przy pomocy [tt][aria-invalid][/tt].[/p]
[p]Po więcej info o WAI-ARIA zapraszam do [url=http://w3c.github.io/html-aria/]specyfikacji [i]ARIA in HTML[/i][/url]. Natomiast [url=https://www.smashingmagazine.com/inclusive-design-patterns/]o dostępności najlepiej pisze Heydon Pickering[/url]. Po polsku polecam [url=http://internet-bez-barier.com/]blog Internet bez barier[/url].[/p]
...
[p]Tutaj warto wspomnieć o głównym konkurencie microdata – [url=http://rdfa.info/]RDFa[/url] – który wygląda dość podobnie, ale jest bardziej XML-friendly i… ładniejszy. Osobiście preferuję go ponad mikrodane.[/p]
...
[p]Dublin Core jest międzynarodowym standardem metadanych (ISO 15836-2003) i służy do opisywania dokumentów. Jest rozwijany i upowszechniany przez organizację Dublin Core Metadata Initiative, mającą na celu stworzenie semantycznej sieci, w której z łatwością można będzie znaleźć dokumenty (pieśń przyszłości). Standard ustala 15 elementów, dzięki którym można dokładniej opisać stronę, lecz nie tylko – praktycznie każdy dokument cyfrowy. Może dzięki temu stanowić zamiennik dla "standardowych" [tt]meta[/tt]. Najczęściej służy jako zamiennik dwóch metatagów: [tt]keywords[/tt] i [tt]description[/tt]. Ich "dublinowe" odpowiedniki to: [tt]dcterms.subject[/tt] i [tt]dcterms.description[/tt].[/p]
...
[p]Na początku [tt]body[/tt] warto dodać link odsyłający nas bezpośrednio do [tt]main[/tt] danej strony (przy użyciu [url=https://tutorials.comandeer.pl/html5-blog.html#artykul-naglowki-jako-punkty-nawigacyjne]opisanych wyżej kotwic[/url]):[/p]
[code=markup]<a href="#tresc" class="focus-only">Przejdź do treści</a>[/code]
...
[p]Oczywiście link będzie czuły na TAB. Inna metoda (bardziej użyteczna i nie powodująca problemów z wydajnością na urządzeniach mobilnych) to [url=https://adaptivethemes.com/using-css-clip-as-an-accessible-method-of-hiding-content/]wykorzystanie [tt]clip[/tt][/url]. Stosuje ją m.in. [url=https://github.com/h5bp/html5-boilerplate/blob/76f6b7d95358ffabb6ac6c4bc8d44a5af35fea8d/dist/css/main.css#L128-L168]H5BP[/url].[/p]
[code=css].focus-only
{
	border: 0;
	clip: rect(0 0 0 0);
...
[list]
	...
	[*] Wszystkie [tt]script[/tt] (oprócz tego dla IE) warto przerzucić na koniec [tt]body[/tt] (zwiększy to szybkość ładowania strony, bo skrypty w [tt]head[/tt] mogą ją [url=https://developer.yahoo.com/performance/rules.html#js_bottom]"blokować"[/url]). Jeśli mamy bardzo dużo JS, ze złożonymi relacjami pomiędzy poszczególnymi plikami, warto rozważyć [url=https://github.com/umdjs/umd]architekturę modułową[/url]. Czy już wspominałem o [url=https://calendar.perfplanet.com/2016/prefer-defer-over-async/]atrybucie [tt][defer][/tt][/url]?
	[*] Można się w ogóle pokusić o serwowanie CSS, JS i obrazków z [url=https://en.wikipedia.org/wiki/Content_delivery_network]CDN[/url]. Jest to jedno z zaleceń Google odnośnie szybkości wczytywania stron: rozłożenie wczytywania na 2 paralelne domeny. Jedna serwuje dynamiczną stronę (czyli PHP i generujemy blogaska), a druga serwuje wszystko, co statyczne. Jednak rozłożenie wczytywania strony to tylko część zalet i bardziej rozbudowane CDN-y korzystają choćby z geolokalizacji, żeby zasysać zasoby z serwera jak najbliżej użytkownika, aby czas wczytywania był jeszcze krótszy. Istnieją także darmowe CDN-y, np. [url=https://www.jsdelivr.com/]jsDelivr[/url]. Bardzo ważnym przeciwskazaniem dla CDN-ów jest fakt, że nie mamy nad nimi kontroli, co sprawia, że [i]de facto[/i] uzależniamy swoje bezpieczeństwo od bezpieczeństwa zewnętrznej usługi. Warto mieć to na uwadze.
...
[list]
	...
	[*] Oprócz RSS można dodać także [url=https://en.wikipedia.org/wiki/Atom_(standard)]Atom[/url], bo to o wiele lepszy format i lepiej się z nim pracuje.
	...
	[*] Jak już jesteśmy przy społecznościówkach, Facebook też serwuje [url=https://davidwalsh.name/facebook-meta-tags]swoje metatagi[/url] a Twtiter [url=https://dev.twitter.com/docs/cards]nie pozostaje mu dłużny[/url].
	[*] Warto też serwować ikonkę strony. Najlepiej mieć podstawową – [tt]favicon.ico[/tt] – w głównym folderze witryny. Wtedy, nawet jeśli nie zamieścimy [tt]link[rel=icon][/tt] w kodzie, przeglądarki sobie ją pobiorą (tak, przeglądarki po prostu na chama szukają ikonki, śląc requesty pod [tt]http://naszastrona.pl/favicon.ico[/tt]). Mając taką podstawową w zapasie, można próbować wcisnąć browserom np coś w PNG właśnie przy pomocy [tt]link[rel=icon][/tt]. IE < 9 i tak tego nie rozumie, uparcie szukając [tt]link[rel=shortcut][/tt], zatem ono dostanie ico a reszta ładny PNG (albo nawet animację w GIF). Do tego dochodzą jeszcze np. [url=https://mathiasbynens.be/notes/touch-icons]ikonki dotykowe dla iUrządzeń[/url] i inne takie, które też można (a jeśli strona ma być dla mobilnych też – nawet trzeba) zamieścić. Są od tego [url=https://realfavicongenerator.net/]odpowiednie narzędzia[/url]. Uwaga! Generowany kod to prawdziwa kobyła. Tak się to kończy, gdy [url=https://css-tricks.com/favicon-quiz/]nie istnieje jeden, powszechny standard[/url].
	[*] W nowszych Windowsach (7 i 8) można sobie [url=https://msdn.microsoft.com/library/gg491732(v=vs.85).aspx]przypiąć stronę[/url] i odpalać ją jak zwykłą aplikację (oczywiście w IE). Podobna opcja [url=https://productforums.google.com/forum/#!topic/chrome/9jHS2dpJX5Q]jest dostępna w Chrome[/url]. Istnieje także [url=http://www.buildmypinnedsite.com/]generator przypinanych stron[/url].
	[*] Jak już przy generatorach jesteśmy, istnieje [url=https://www.pwabuilder.com/generator]generator PWA[/url], powstały z dawnego projektu Manifold.js.
...
[list]
	...
	[*] [url=https://w3c.github.io/aria/]Specyfikacja WAI-ARIA[/url]
	[*] [url=https://w3c.github.io/aria-practices/]Oficjalny poradnik dla chcących bawić się WAI-ARIA[/url]
	[*] [url=http://microformats.org/wiki/Main_Page]Specyfikacje i oficjalne materiały o mikroformatach[/url]
	[*] [url=https://w3c.github.io/microdata/]Specyfikacja microdata[/url]
	[*] [url=http://dublincore.org/]Informacje o Dublin Core[/url]
	[*] [url=http://w3c.github.io/html/semantics-embedded-content.html#alt-text]Oficjalna część specyfikacji HTML5, opsiująca użycie atrybutu [tt][alt][/tt][/url]
	[*] [url=http://usecases.responsiveimages.org]Oficjalna informacja o responsywnych obrazkach[/url]
	[*] [url=https://html.spec.whatwg.org/multipage/embedded-content.html#the-picture-element]Definicja tagu [tt]picture[/tt] w HTML Living Standard[/url] (jest dokładniejsza i lepiej odzwierciedla implementacje w przeglądarkach niż wersja w HTML 5.x)
	[*] [url=https://tools.ietf.org/html/rfc7540]Specyfikacja HTTP/2[/url]
	[*] [url=https://w3c.github.io/webappsec-csp/]Specyfikacja Content Security Policy[/url]
...
[list]
	...
	[*] [url=https://lea.verou.me/]Dawny perfekcyjny przykład bloga w HTML5[/url]
	[*] [url=http://diveintohtml5.info/extensibility.html]Microdata[/url]
	[*] [url=http://html5doctor.com/microdata/]Doktorzy o microdata[/url]
	[*] [url=https://kurs.browsehappy.pl/HTML/DublinCore]DublinCore[/url]
	[*] [url=https://webaim.org/techniques/alttext/]Techniki dobierania odpowiedniego atrybutu [tt][alt][/tt][/url]
	[*] [url=https://alistapart.com/article/waiaria]WAI-ARIA[/url]
	...
	[*] [url=https://webroad.pl/inne/3722-progressive-enhancement-zapomniany-fundament]Progressive Enhancement[/url]
	[*] [url=https://webroad.pl/html5-css3/3925-validate-or-not-to-validate-that-is-the-question]Krótki artykuł o walidacji[/url]
[/list]

[h3="narzedzia"]Narzędzia[/h3]
[list]
	[*] [url=https://validator.w3.org/nu]Oficjalny walidator[/url] dla wersji standardu od W3C
	[*] [url=https://checker.html5.org]Oficjalny walidator[/url] dla wersji standardu od WHATWG
	[*] [url=https://wave.webaim.org/]Narzędzie do sprawdzania dostępności strony[/url]
	[*] [url=https://www.webpagetest.org/]Narzędzie do mierzenia wydajności strony[/url]
	[*] [url=https://developers.google.com/speed/pagespeed/insights/]Inne narzędzie do mierzenia wydajności strony[/url]
	...
	[*] [url=https://caniuse.com/]Prosty sposób na sprawdzenie wsparcia HTML5 w różnych przeglądarkach[/url]
	...
	[*] [url=https://html5test.com/]Test wsparcia dla HTML5[/url]
	[*] [url=https://csp-evaluator.withgoogle.com/]Walidator reguł CSP[/url]
	[*] [url=https://www.webbloatscore.com/]Miernik "otyłości" strony[/url] (radzę nie traktować za poważnie!)
	...
	[*] [url=https://www.freedomscientific.com/Products/Blindness/JAWS]Czytnik ekranowy JAWS[/url]
	[*] [url=https://www.nvaccess.org/]Czytnik ekranowy NVDA[/url]
	[*] [url=https://www.apple.com/accessibility/mac/vision/]Czytnik ekranowy VoiceOver[/url]
[/list]
