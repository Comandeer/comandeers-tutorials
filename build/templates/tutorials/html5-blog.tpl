...
[p]Dobra, akapitów się (jeszcze) zmienić nie da, więc treść mamy z głowy :D Tagi i komentarze zostają bez większych zmian, [s]jedynie [tt]h3[/tt] zmienia się w [tt]h2[/tt]. Czemu?[/s] Przypatrzmy się [url=http://tutorials.comandeer.pl/tutorials/html5-blog/outlinehtml5.png]outline[/url]. Zaskoczeni? (Pewnie tak, bo do części kodu jeszcze nie doszliśmy :D). Jak widać w HTML5 outline jest liczony dla każdej sekcji i artykułu oddzielnie, tzn. [tt]h1[/tt] w artykule będzie niżej niż [tt]h1[/tt] w dokumencie. Dla porównania [url=http://tutorials.comandeer.pl/res/html5-blog/outlinehtml4.png]outline z HTML4[/url]. O wiele wygodniej, prawda?[/p]
[p]Co do liczenia outline'u w HTML5 warto także dodać, że można go budować obecnie tylko przy pomocy [tt]h1[/tt] (i wymuszać hierarchię przez odpowiednie wykorzystywanie [tt]section[/tt] czy [tt]article[/tt]). Jednak obecnie wiele osób zaleca połączenie obydwu metod (tagi sekcjonujące + różnicowanie ważności nagłówków), gdyż nie wszystkie przeglądarki zaimplementowały już nowy algorytm i przez to np [url=http://en.wikipedia.org/wiki/Assistive_Technology]AT[/url] mogą dostać spłaszczoną hierarchię nagłówków (co raczej jest średnim pomysłem). Te obawy potwierdzają [url=http://blog.paciellogroup.com/2013/10/html5-document-outline/]najnowsze doniesienia[/url], z których wynika, że nikt nie zaimplementował jeszcze nowego sposobu liczenia nagłówków. Dlatego też kod bloga został poprawiony tak, by odzworowywać hierarchię nagłówków przy pomocy sekcji + starej hierarchi z HTML4.[/p]
