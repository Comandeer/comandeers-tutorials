...
[p]Owszem, w VanillaJS nie jestem w stanie uzyskać tak wygodnej obsługi DOM. Ale tu z pomocą przychodzi nam [url=http://leaverou.github.com/chainvas/]Chainvas[/url] - małe cudeńko, które jest w stanie złańcuchować de facto wszystko. i dzięki temu można spokojnie napisać[/p]
...
[code=javascript]$(document).on('click', '.clicky', function(e)
{
	//awesome stuff
});[/code]
[p]w czystym JS też jest prosto[/p]
[code=javascript]document.addEventListener('click', function(e)
{
...
[p=warning][b]UWAGA![/b] Podany tutaj przykład rozszerza prototyp [tt]Object[/tt], co może być [url=http://www.nczonline.net/blog/2010/03/02/maintainable-javascript-dont-modify-objects-you-down-own/]dość kiepskim pomysłem[/url] (a przynajmniej w takiej formie, bez ustawienia enumeracji wartości na fałsz)[/p]
[code=javascript]Object.prototype.clone = function()
{
	return this.cloneNode ? this.cloneNode(this) : JSON.parse(JSON.stringify(this));
};
console.log(document.createElement('a').clone());[/code]
[p]Prosta i przyjemna sztuczka ;)[/p]

[h3="extend"][tt].extend[/tt][/h3]
[p]To już nie jest do końca DOM, ale może się tu nadać ;)[/p]
[p]jak wiadomo, w jQ mamy metodę [tt].extend[/tt] uzupełniającą nam obiekt[/p]
...
[p]Otóż wrapper w jQ jest cholernie wygodny i wgl. Jednak jeśli chcemy mieć dostęp do bardziej zaawansowanych własności obiektu xhr (np. [tt]xhr.upload.progress[/tt]) [url=https://github.com/malsup/form/blob/master/jquery.form.js#L258]trzeba użyć natywnego obiektu[/url].[/p]
